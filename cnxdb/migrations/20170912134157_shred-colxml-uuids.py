...
FIND_BOOK_META=plpy.prepare("SELECT * FROM modules WHERE module_ident = $1", ("int",))
NODE_INS=plpy.prepare("INSERT INTO trees (parent_id, documentid, childorder)"
...
SUBCOL_ACL=plpy.prepare("INSERT INTO document_controls (uuid, licenseid)"
...
SUBCOL_INS=plpy.prepare("INSERT into modules (portal_type, moduleid, name, uuid,"
...
SUBCOL_NEW_VERSION=plpy.prepare("INSERT into modules (portal_type, moduleid, name, uuid,"
"    abstractid, version, created, revised,"
"    licenseid, submitter, submitlog, stateid,"
"    parent, language, doctype,"
"    authors, maintainers, licensors, parentauthors,"
...
drop function shred_collxml(fid int, docid int);

drop function shred_collxml(doc text);
...
drop function shred_collxml(fid int, docid int, force bool);

drop function shred_collxml(doc text, docid int);
