...
def should_run(cursor):
    # Regenerate the slugs as part of changing slug generation code per:
    # https://github.com/openstax/cnx/issues/972
    #
    # We're specifically looking for HS Physics where slugs are have a prefix
    # of "chapter" which we need to remove. We'll gate the run based on
    # a known "chapter-2-motion-in-one-dimension" slug
    cursor.execute("""\
        SELECT slug FROM trees
        JOIN modules ON trees.documentid = modules.module_ident
        WHERE modules.uuid = 'd9956deb-accc-5d91-aa9a-81ad8053d29f'
        and slug is not NULL LIMIT 1""")
    result = cursor.fetchone()
    if result:
        return result[0] == 'chapter-2-motion-in-one-dimension'
    return False
